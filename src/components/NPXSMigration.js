    ...
    render() {
        return (
            <div id="content" className="mt-3">
                ...
                <div className="card mb-4 card-body" >
                <h3 className="table table-borderless text-muted text-center">Claimable NPXSXEM Token</h3>&nbsp;                 
                <table className="table">
                    ...
                    <tbody id="claimList">
                        {this.props.migrator.map((migratorInfo, key) => {
                            let date = new Date(migratorInfo.unlockTime * 1000);
                            return (
                                <tr key={key}>
                                    {/* <th scope="row">{this.props.account}</th> */}
                                    <td>{this.props.account}</td>
                                    <td>{window.web3.utils.fromWei((migratorInfo.migrateBalance*0.08).toString(), 'Ether')} NPXSXEM</td>
